...
def process_image(image, task, tile=None, tile_overlap=32):

    def load_img_from_array(image_array):
        return cv2.cvtColor(image_array, cv2.COLOR_BGR2RGB)

    def save_img(filepath, img):
        cv2.imwrite(filepath, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

    def load_gray_img_from_array(image_array):
        return np.expand_dims(cv2.cvtColor(image_array, cv2.COLOR_BGR2GRAY), axis=2)
    ...
    with torch.no_grad():
        ...
        if task == 'Gaussian_Gray_Denoising':
            img = load_gray_img_from_array(image)
        else:
            img = load_img_from_array(image)
