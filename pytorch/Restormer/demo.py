...
def restore_image(image_path, task, result_dir='./temp/', tile=None, tile_overlap=32):
    out_dir = os.path.join(result_dir, task)
    os.makedirs(out_dir, exist_ok=True)

    extensions = ['jpg', 'JPG', 'png', 'PNG', 'jpeg', 'JPEG', 'bmp', 'BMP']

    if any([image_path.endswith(ext) for ext in extensions]):
        files = [image_path]
    else:
        files = []
        for ext in extensions:
            files.extend(glob(os.path.join(image_path, '*.'+ext)))
        files = natsorted(files)

    if len(files) == 0:
        raise Exception(f'No files found at {image_path}')
    ...
    with torch.no_grad():
        for file_ in tqdm(files):
            ...
            if task == 'Gaussian_Gray_Denoising':
                save_gray_img((os.path.join(out_dir, f+'.jpg')), restored)
            else:
